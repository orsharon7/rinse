...
func TestIsPro_TrueWhenConfigSaysYes(t *testing.T) {
	tmp := t.TempDir()
	orig := os.Getenv("HOME")
	os.Setenv("HOME", tmp)
	defer os.Setenv("HOME", orig)
	...
	if !IsPro() {
		t.Error("IsPro() should return true when config has pro=true")
	}
}

func TestIsPro_FalseByDefault(t *testing.T) {
	tmp := t.TempDir()
	orig := os.Getenv("HOME")
	os.Setenv("HOME", tmp)
	defer os.Setenv("HOME", orig)
	...
	if IsPro() {
		t.Error("IsPro() should return false when no config exists")
	}
}
