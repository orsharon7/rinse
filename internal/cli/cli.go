...
func runPredictCmd(args []string) {
	...
	// the user is Pro. Keep _ = interactive until RIN-208 lands.
	_ = interactive
	...
	// ── Doc-drift analysis ────────────────────────────────────────────────────
	//
	// When --doc-drift is active (Pro gate already enforced at TryDispatch),
	// run the LLM-backed drift detector and merge its findings into the report.
	if docDrift && predict.IsDocDriftEnabled(docDrift) {
		...
		if diffErr != nil {
			...
			if driftErr != nil {
				...
				fmt.Fprintf(os.Stderr, "[doc-drift] LLM calls used: %d/10\n", driftReport.LLMCallCount)
...
// fetchDiff returns the unified diff for staged changes or a PR.
// Used by the doc-drift detector to analyse the same source as predict.Run.
...
COMMANDS
  ...
  rinse init
    ...
    Also offers to generate .github/copilot-instructions.md to reduce Copilot
    review noise. RINSE detects your primary language (Go, TypeScript, JavaScript,
    Python, Rust) and writes a tailored template that tells Copilot what to focus
    on (bugs, security, error handling) and what to skip (style, formatting,
    naming). Commit this file — Copilot uses it for every PR in the repo.
  ...
  rinse predict [<pr>] [--repo <owner/repo>] [--json] [--no-log]
    ...
    --doc-drift           (Pro) Run LLM-backed documentation drift detection.
                          ...
                          Uses Copilot API only (≤10 calls/run, <3s per call).
                          Detects: stale godoc, missing godoc on exported
                          symbols, README examples referencing outdated APIs.
...
SESSION DATA
  ...
  rinse stats --predict
    ...
    (logged by rinse predict) with actual session patterns.

    Output includes:
      • Rolling 10-PR hit rate   — accuracy over the last 10 predict runs
      • All-time hit rate         — accuracy across all recorded events
    ...
    that started within 10 minutes of the prediction run, then counting how many
    predicted patterns appeared in that session's Patterns list.
...
FILES
  ...
  .github/copilot-instructions.md
                  Optional Copilot context file generated by 'rinse init'.
                  Tells Copilot what to focus on (bugs, security, error
  ...
  Linux:  ~/.config/rinse/config.json
                  ...
                  keyed by repository. Written by the settings screen
                  (press s in the PR picker). Not committed — settings
...
EXAMPLES
  ...
  # Set up RINSE for a new repo (creates .rinse.json and optionally
  # .github/copilot-instructions.md with a language-aware template)
  rinse init
