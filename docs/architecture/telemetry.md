...
| DB CHECK constraint vs runner outcome vocabulary mismatch | DB allows `('merged','closed','open','failed','approved','error','aborted','max_iterations','clean','dry_run')` for fresh installs; `open` and `failed` are written as raw string literals by the runner; `stats.go` defines typed constants for JSON-session compatibility | **Fixed** — migrations 006 and 008 expanded constraint to full set; existing installs without the CHECK are unaffected |
| `clean` and `dry_run` missing from DB CHECK | `OutcomeClean` and `OutcomeDryRun` defined in `stats.go` but not in original schema constant | **Fixed** — migration 008 adds them; runner does not currently write these to DB but schema is now permissive |
| `synced_at` column missing | Needed for Phase 2 sync tracking | Add as migration when sync ships |
