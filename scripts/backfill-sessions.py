...
for log_path in sorted(LOG_DIR.glob("rinse-pr*.log")):
    ...
    try:
        ...
        if comments_fixed == 0:
            comments_fixed = len(re.findall(r'reply_to.*?comment', content, re.IGNORECASE))
        if comments_fixed == 0:
            # Fallback: count "Now reply to" lines
            comments_fixed = len(re.findall(r'Now reply to', content))

        # Outcome
        if 'Merged PR' in content or ('squash' in content.lower() and 'merged' in content.lower()):
            outcome = 'merged'
        ...
        repo_match = re.search(r'repo[=\s]+([a-z0-9_\-]+/[a-z0-9_\-]+)', content, re.IGNORECASE)
        ...
        try:
            conn.execute("""
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
            """, (session_id, repo, pr_num, pr_title, branch,
                  start_dt.isoformat(), end_dt.isoformat(),
                  duration, estimated_saved, iterations,
                  comments_fixed, outcome, log_path.name))
            conn.commit()
            sessions_inserted += 1
            print(f"  ✅ PR #{pr_num:3d} | {duration//60:3d}m | {iterations} iters | {comments_fixed} comments | {outcome}")
...
# Summary stats
print(f"\n{'─'*45}")
cur = conn.execute("SELECT COUNT(*), SUM(duration_seconds), SUM(total_comments_fixed), SUM(estimated_time_saved_seconds), SUM(iterations) FROM sessions")
row = cur.fetchone()
...
print(f"  Avg iterations:    {(iters/total):.1f}" if total else "  Avg iterations:   —")
